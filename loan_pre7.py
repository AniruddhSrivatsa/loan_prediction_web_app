...
edu=st.selectbox("",["Graduated","Not Graduated"])
if edu is not None:
  if edu=="Graduated":
    edu_int=1
...
sem=st.selectbox("Self Employed",["Yes","No"])
if sem is not None:
  if sem=="Yes":
    sem_int=1
...
app_inc=st.number_input("Applicant's income in dollars")/1000
st.subheader("Co-applicant's income :")
coapp_inc=st.number_input("Co-applicant's income in dollars")/1000
...
ada_boost=pickle.load(open(r"C:\Users\Aniruddh Srivatsa\Downloads\ada.pkl","rb"))
...
reason=st.sidebar.radio("What is the reason for applying for loan ?",["Financial Problem","Education",
